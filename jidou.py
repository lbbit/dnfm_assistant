...
def select_action_with_probability(action_weights):
    actions = [
        ...
        game_controls.skill9_jidou1,
        game_controls.skill9_jidou2
    ]
    # 创建一个包含动作和其对应权重的列表
...
def main(loop_count=1, do_mission=False, action_weights=None):
    ...
    for i in range(loop_count):
        ...
        game_controls.jidou_next_page(100)
        sleep_time = 5
    ...
    if do_mission:
        ...
        print(f"END: Claiming missions.")
        game_controls.jidou_claim_mission(100)
        game_controls.jidou_claim_mission(100)
        game_controls.jidou_claim_mission(100)
        time.sleep(2)
        game_controls.claim_button(100)
        sleep_time = 3
        print(f"END: Sleeping for {sleep_time} seconds after claiming missions.")
        time.sleep(sleep_time)
        game_controls.return_button(100)
        game_controls.return_button(100)
